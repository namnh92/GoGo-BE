...
export class CmsOpsService {
  ...
  // --- bulk import (CMS-009, FR-CMS-009) -----------------------------------

  /**
   * JSON bulk import with dry-run + per-row error report. CSV file upload
   * rides on object storage (blocked on R2 credentials — GoGo-BE#70).
   */
  async runPlaceImport(
    adminId: string,
    input: {
      dryRun: boolean;
      rows: {
    ...
    for (const [i, row] of input.rows.entries()) {
      if (!row.name || row.lat === undefined || row.lng === undefined) {
        errors.push({ row: i, message: 'name/lat/lng required' });
        continue;
      }
      if (Math.abs(row.lat) > 90 || Math.abs(row.lng) > 180) {
        errors.push({ row: i, message: 'invalid coordinates' });
        continue;
      }
      if (!input.dryRun) {
        await this.db.insert(schema.places).values({
          ...
          status: 'draft',
          geom: { x: row.lng, y: row.lat },
          areaKey: row.areaKey ?? null,
        });
      }
      created += 1;
    }

    const report = { total: input.rows.length, created, errors };
    const status = errors.length === 0 ? 'succeeded' : created > 0 ? 'partial_failure' : 'failed';
    await this.db
      .update(schema.importJobs)
      .set({ status, report, finishedAt: sql`now()` })
      .where(eq(schema.importJobs.id, job!.id));
    await this.audit(adminId, 'import_job.finished', 'import_job', job!.id, {
      status,
      dryRun: input.dryRun,
    });
    return { jobId: job!.id, status, report };
  }
