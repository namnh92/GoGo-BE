 ...
 * 3. **It cannot invent a unit.** A mapping row is promoted to a canonical
...
export class AdministrativeImportService {
  ...
  async importPinnedSnapshot(options: { overrideRevision?: number } = {}): Promise<ImportReport> {
    ...
    await insertChunked(allUnits, (chunk) =>
      this.db.insert(schema.administrativeUnits).values(
        chunk.map((u) => ({
          datasetVersionId,
          code: u.code,
          name: u.name,
          ...
          source: u.status === 'ACTIVE' ? current.source.repository : historical.source.repository,
    ...
    await insertChunked(canonical, (chunk) =>
      this.db.insert(schema.administrativeUnitChanges).values(
        chunk.map((c) => ({
          datasetVersionId,
    ...
    await insertChunked(quarantined, (chunk) =>
      this.db.insert(schema.administrativeMappingQuarantine).values(
    ...
    return {
      ...
      counts: {
        ...
        legacyDistricts: historicalUnits.units.filter((u) => u.level === 'LEGACY_DISTRICT').length,
