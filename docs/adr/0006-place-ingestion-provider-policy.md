...
| Refresh bookkeeping (`refresh_priority`, `refresh_attempts`, `last_refresh_attempt_at`, `last_refresh_error_code`) | `place_provider_sources`                                                                                                | **allowed**                          | GoGo-generated metadata about our own calls (#340). `last_refresh_error_code` holds Google's canonical status (`NOT_FOUND`, `INVALID_ARGUMENT`) or `EMPTY_ANSWER` — a status code about our request, never a payload or a message.                                          |
