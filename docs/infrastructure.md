...
| `place_link_identity_conflict_total`                 | `source`                                     | link chia sẻ nêu hai định danh Google khác nhau (`place_id` vs `ftid`) — từ chối, không đoán (#505)                                                                                                   |
| `place_link_cid_lookup_total`                        | `result`                                     | tra định danh bằng CID trên Text Search Pro (#505): `matched`/`unmatched`/`not_found` — `unmatched` là link trỏ tới nơi search không trả về                                                           |
| `place_link_resolution_stage_seconds`                | `stage`                                      | thời gian từng chặng resolve link: `expand`/`search`/`details` (#505) — để định vị một resolve chậm                                                                                                   |
| `place_submission_provider_preview_total`            | `result`                                     | kiểm duyệt viên xem dữ liệu Google của một đóng góp (#528) — một Details `quality`, đếm tách khỏi lần fetch lúc duyệt                                                                                 |
...
| `cms_place_create_provider_enrichment_total`         | `result`                                     | một Details `quality` khi editor tạo Place từ link (PI-BE-021, ADR-0020); `unavailable` tăng = Place mới thiếu fact provider                                                                          |
...
| `media_cleanup_pending`                              | —                                            | gauge: hàng dọn dẹp media đang chờ (worker tick mỗi phút)                                                                                                                                             |
| `media_cleanup_dead_lettered`                        | —                                            | gauge: hàng dọn dẹp đã dead-letter sau 6 lần, cần người xử lý                                                                                                                                         |
| `avatar_set_total`                                   | `outcome`                                    | PUT /me/avatar kết quả (ADR-0022); `outcome` = `ok`/`rejected`/`unavailable`/`error`                                                                                                                  |
| `avatar_set_duration_seconds`                        | —                                            | thời gian đọc + xử lý sharp + ghi bucket public của một avatar                                                                                                                                        |
| `avatar_removed_total`                               | —                                            | DELETE /me/avatar thành công                                                                                                                                                                          |
| `media_cleanup_enqueued_total`                       | —                                            | hàng media_cleanup_queue được ghi (cùng transaction với thay đổi avatar)                                                                                                                              |
| `media_cleanup_attempt_total`                        | `bucket`, `outcome`                          | mỗi lần xử lý một hàng dọn dẹp; `outcome` = `done`/`retried`/`dead_lettered`/`referenced` (object còn là avatar sống, bị từ chối xoá)                                                                 |
...
| `push_delivery_sent_total`                           | `kind`                                       | message provider đã tạo (#193); tăng theo số message, không theo số lần gọi. Từ #594 một event outbox gọi provider một lần cho mỗi nhóm ngôn ngữ (`vi`, `en`), nên cộng dồn theo từng lần gọi         |
| `push_delivery_no_target_total`                      | `kind`                                       | provider trả 200 nhưng không có subscription nào trong request — không phải gửi, không phải lỗi; tính theo từng lần gọi provider (mỗi nhóm ngôn ngữ, #594)                                            |
| `push_delivery_unknown_user_total`                   | `kind`                                       | user id provider không biết (chưa từng login/đã logout) — không phải lỗi gửi                                                                                                                          |
| `push_delivery_failed_total`                         | `kind`                                       | provider từ chối vĩnh viễn (credential/payload); lỗi tạm thời đi qua outbox retry. Tăng 1 cho mỗi lần gọi bị từ chối — từ #594 một event có thể gọi hai lần (`vi`, `en`), nên một event có thể tăng 2 |
...
| `push_subscription_registrations_total`              | `result`                                     | ghi nhận thiết bị có subscription push (#515): `registered` / `refused` / `unverified` / `revoked`. Đây là nguồn của audience chiến dịch — `registered` tụt về 0 nghĩa là chiến dịch sắp không tới ai |
